...
df_total = pd.read_pickle(f'{MAIN_DIR}/df_tumour_expression.pkl')
...
def clean_data(df_total, num_of_genes_to_select=None):
    ...
    ###############################
    df_clean = df_clean.replace({"grade": {'1': 1}})
    df_clean = df_clean.replace({"grade": {'2': 2}})
    df_clean = df_clean.replace({"grade": {'3': 3}})
    df_clean = df_clean.replace({"grade": {'--': np.nan}})
    
    
    df_clean = df_clean.replace({"grade": {3.0: 3}})
    df_clean = df_clean.replace({"grade": {4.0: 3}})
    df_clean = df_clean.replace({"grade": {1.0: 1}})
...
df_total = df_total.replace({'er':{' NA':np.nan,' EV':np.nan, '1':1, '0':0},
                               ...
                               'tumor-size(mm)':{'>= 2.0':2.5, '< 2.0':1.0, ' --':np.nan, ' NA':np.nan },
                                'subtype':{' NA':np.nan, 'TN': np.nan},
                                 'age-at-diagnosis(yrs)':{'--':np.nan, '>=50':60, '<50':40}})
...
df_total.loc[df_total['tumor-size(mm)'] < 2.0, 'tumor-size(mm)'] = 0
df_total.loc[df_total['tumor-size(mm)'] >= 2.0, 'tumor-size(mm)'] = 1
...
def balance_df(X, y, random_state=None):
    ...
    # Return the resampled dataset as a DataFrame and Series
...
def run_all(df):
    ...
    for exp_id in range(0, NUM_RUNS):
        ...
        model.fit(train_df, y_balanced, eval_set=eval_set, verbose=False)
...
df_total = df_total.sort_values(by='gain', ascending=False)
df_total.head(70)


df_total[:70].to_csv('CGM_biomarkers.csv')


list_fati = df_total.index.tolist()[:70]
len(list_fati)
list_elmira = ["BIRC5","CDC20","CCNB2","PTTG1","AURKA","SLC7A5","TPX2","CENPN","MCM10","UBE2C","NME5","MELK","CENPA","KIF2C","TRIP13","SCUBE2","STC2","PTPRT","LINC00472","STK32B","RRM2","BBOF1","EXO1","MKI67","WDR19","E2F8","AURKB","DRC3","NAT1","ERBB4","HJURP","KIF13B","PCSK6","CACNA1D","SLC25A12","LRP8","ILST","PGR","OSBPL1A","TMC5","ORC6","CIRBP","STAT1","ESD","MYBL2","PRR22","CLMN","NCAPH","PDZRN3","TUBA4A","DNAJC12","CX3CR1","IFI44L","TBC1D9","HSPB1","TMEM132A",   "PSD3","NTRK2","OR7E36P","GLRB","STARD13","ABAT","MATN3","ADRA2A","ADGRG1","NAV2","FMO5","GINS1","RPP40","LAMP5"]
first_n = 70
len(set(list_elmira[:first_n]) - set(list_fati[:first_n]))


genes_list = df_total.index.tolist()
with open(f'{MAIN_DIR}/genes_list.pkl', 'wb') as fp:
    pickle.dump(genes_list, fp)
